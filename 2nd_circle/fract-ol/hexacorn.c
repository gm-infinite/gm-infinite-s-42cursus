...
/*   Updated: 2025/01/25 15:54:20 by kuzyilma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

static void	ft_hexacorn_it(t_coordinate count, t_data *img, int color[])
{
	t_complex	z;
	t_complex	c;
	int			i;
	...
	while (i < ITERATION - 1)
	{
		z = add_complex(mult_complex(get_complex(z.x, -z.y), \
		square_complex(square_complex(get_complex(z.x, -z.y)))), c);
...
void	hexacorn(t_ft_window *wnd)
{
	...
	while (count.x < WINDOW_WIDTH)
	{
		...
		{
			ft_hexacorn_it(count, &(wnd->img), wnd->color);
			count.y++;
		}
		count.x++;
	}
