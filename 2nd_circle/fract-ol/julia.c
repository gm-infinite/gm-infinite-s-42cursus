...
/*   Updated: 2025/01/25 15:55:15 by kuzyilma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

static void	ft_julia_it(t_complex c, t_coordinate count, t_data *img, \
int color[])
{
	t_complex	z;
	int			i;
	...
	i = 0;
	z.x = (ft_map(count.x, -2 * img->mvmt.zoom + img->mvmt.x_offset, \
	+2 * img->mvmt.zoom + img->mvmt.x_offset, WINDOW_WIDTH));
	z.y = (ft_map(count.y, +2 * img->mvmt.zoom + img->mvmt.y_offset, \
